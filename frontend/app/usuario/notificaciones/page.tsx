...
export default function UsuarioNotificacionesPage() {
  ...
  const getIcono = (tipo: string) => {
    ...
    if (tipo === "favorito" || tipo === "terreno_guardado") {
      return <Heart size={18} className="text-[#22341c]" />;
    }
  ...
  const getTipoLabel = (tipo: string) => {
    ...
    if (tipo === "terreno_guardado") return "Guardado";
    if (tipo === "lead_nuevo") return "Contacto";
    return tipo || "Aviso";
  };
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-[#817d58]">
              Mantente al tanto de respuestas, mensajes y movimientos relevantes
              en tus interacciones dentro de la plataforma.
            </p>
          </div>
        ...
        {cargando ? (
          ...
          <div className="rounded-[1.8rem] border border-[#817d58]/12 bg-white p-10 text-center shadow-sm">
            ...
            <h2 className="mt-5 text-xl font-semibold text-[#22341c]">
              No hay notificaciones por mostrar
            </h2>
            <p className="mt-2 text-sm text-[#817d58]">
              Cuando recibas respuestas o haya actividad relevante en tu cuenta,
              aparecerá aquí.
            </p>
          </div>
        ) : (
